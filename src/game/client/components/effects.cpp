...
void EFFECTS::powerupshine(vec2 pos, vec2 size)
{
	...
	if (config.gfx_effects)
	{
		...
		p.pos = pos + vec2((frandom() - 0.5f) * size.x, (frandom() - 0.5f)
				* size.y);
		p.vel = vec2(0, 0);
		if (config.gfx_eyecandy)
		{
			p.gravity = -100;
			p.life_span = 1.3f;
			p.color = vec4(frandom() * 0.5f + 0.5f, frandom() * 0.5f + 0.5f,
					frandom() * 0.5f + 0.5f, 0.8f);
		}
		else
		{
			p.gravity = 500;
...
void EFFECTS::weaponshine(vec2 pos, vec2 size)
{
	...
	p.spr = SPRITE_PART_SLICE;
	p.pos = pos
			+ vec2((frandom() - 0.5f) * size.x, (frandom() - 0.5f) * size.y);
	p.vel = vec2(0, 0);
	p.gravity = -100;
	p.life_span = 7.0f / 10.0f;
	//p.color = vec4(0.15f,0.5f,0.7f,0.5f);
	p.color = vec4(frandom() * 0.5f + 0.5f, frandom() * 0.5f + 0.5f, frandom()
			* 0.5f + 0.5f, 0.5f);
	p.start_size = 18.0f;
	p.end_size = 0;
	p.rot = frandom() * pi * 2;
...
void EFFECTS::redflagshine(vec2 pos, vec2 size)
{
	...
	p.set_default();
	p.spr = SPRITE_PART_SLICE;
	p.pos = pos + vec2((frandom() - 0.5f) * size.x / 2.0f, (frandom() - 0.5f)
			* size.y);
	p.vel = vec2(0, 0);
	p.gravity = frandom() - 170.0;
	p.life_span = 7.0f / 10.0f;
	//p.color = vec4(0.85f,0.2f,0.2f,1.0f);
	p.color = vec4(frandom() * 0.5f + 0.5f, frandom() * 0.5f, frandom() * 0.5f,
			0.8f);
	p.start_size = 15.0f;
	p.end_size = 0;
	p.rot = frandom() * pi * 2;
...
void EFFECTS::blueflagshine(vec2 pos, vec2 size)
{
	...
	p.color = vec4(frandom() * 0.5f, frandom() * 0.5f, frandom() * 0.5f + 0.5f,
			0.8f);
	p.start_size = 15.0f;
	p.end_size = 0;
	p.rot = frandom() * pi * 2;
...
void EFFECTS::smoketrail(vec2 pos, vec2 vel)
{
	...
	if (config.gfx_effects)
	{
		...
		if (config.gfx_eyecandy)
		{
			...
			p.color = vec4(frandom() * 0.5f + 0.5f, frandom() * 0.5f + 0.5f,
					frandom() * 0.5f + 0.5f, frandom() * 0.5f + 0.5f);
		}
		else
		{
...
void EFFECTS::playerdeath(vec2 pos, int cid)
{
	...
	if (cid >= 0)
	{
		const SKINS::SKIN *s = gameclient.skins->get(
				gameclient.clients[cid].skin_id);
		if (s)
	...
	if (config.gfx_effects)
	{
		for (int i = 0; i < 64; i++)
		{
			...
			int rcolor = rand() % 6;
			if (!config.gfx_eyecandy) rcolor = 0;
			if (rcolor == 1)
			{
...
void EFFECTS::explosion(vec2 pos)
{
	...
	for (int y = -8; y <= 8; y++)
		for (int x = -8; x <= 8; x++)
		{
			...
			gameclient.flow->add(pos + vec2(x, y) * 16, normalize(vec2(x, y))
					* 5000.0f * a, 10.0f);
		}
	...
	for (int i = 0; i < 24; i++)
	{
		...
		p.gravity = frandom() * -800.0f;
		p.friction = 0.4f;
		p.color = mix(vec4(0.75f, 0.75f, 0.75f, 1.0f), vec4(0.5f, 0.5f, 0.5f,
				1.0f), frandom());
		gameclient.particles->add(PARTICLES::GROUP_GENERAL, &p);
	}
}
